...
class FormXIIIApiService extends BaseFormApiService
{
    public function fetch(int $tenantId, int $branchId, int $month, int $year): array
    {
        ...
        try {
            ...
            Log::info('FORM XIII: Fetched from contract_labour', ['count' => count($rows)]);

        } catch (\Exception $e) {
            Log::error('FORM XIII: contract_labour query failed', ['error' => $e->getMessage()]);
            $rows = [];
        }

        // If no data from contract_labour, try contract_labour_deployment
        if (empty($rows)) {
            try {
                ...
                Log::error('FORM XIII: contract_labour_deployment query failed', ['error' => $e->getMessage()]);
                $rows = [];
            }
        }
